...
def getLen(url):
    try:
        ...
        span_total = sum([len(tag.text) for tag in span_tags]) if span_tags != None else 0
        p_total = sum([len(tag.text) for tag in p_tags]) if p_tags != None else 0
        td_total = sum([len(tag.text) for tag in td_tags]) if td_tags != None else 0
        btn_total = sum([len(tag.text) for tag in btn_tags]) if btn_tags != None else 0
...
def resultUrl():
    if request.method == 'POST':
        query = request.values.get('urls')
    result = []
    querys = json.loads(query)
    for j in querys:
        index = count_indexed_pages(j['link'])
        # result.append({'url': j['link'], 'len': getLen(j['link'])})
        print(({'url': j['link'], 'len': getLen(j['link']), 'index': index}))
        result.append({'url': j['link'], 'len': getLen(j['link']), 'index': index})
